...
export async function createApplication(
  ...
  if (options.staticAssets !== false) {
    ...
    app.use(mount(staticAssetsOptions.prefix || '/', koaStatic(Path.join(root, staticAssetsOptions.root), staticAssetsOptions)));
  }
  ...
  if (options.swagger !== false) {
    ...
    debug('应用swagger接口文档 访问路劲：%s', swaggerOptions.url);
